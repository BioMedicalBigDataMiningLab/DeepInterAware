...
#         ag = self.ag_extractor(ag_token)  #batch*max_length*out_channels,batch*max_length*out_channels
        ...
        #Bindingsite task
        ab = self.ab_extractor(ab_token)   # [B, L_ab, C]
        ag = self.ag_extractor(ag_token)   # [B, L_ag, C]

        _, L_ab, _ = ab.shape
        _, L_ag, _ = ag.shape

        ab_mask = ab_mask[:, :L_ab]
        ag_mask = ag_mask[:, :L_ag]

        ab_mask_expanded = ab_mask.unsqueeze(-1).expand(-1, -1, self.channel)
        ag_mask_expanded = ag_mask.unsqueeze(-1).expand(-1, -1, self.channel)

        ab_label = ab_label[ab_mask.bool()]
        ag_label = ag_label[ag_mask.bool()]
...
def load_AM_model(yml,model_path,gpu=0):
    device = torch.device(f'cuda:{gpu}' if torch.cuda.is_available() else 'cpu')
    cfg = get_cfg_defaults()
    cfg.merge_from_file(yml)
    encoder = DeepInterAware(cfg)
